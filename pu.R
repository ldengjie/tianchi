...
#IOlist<-list(c(0),c(106),c(0),c(106),c(0),c(33),c(0),c(0));
#zfb1<-exp(zfb1)
#zfb2<-exp(zfb2)
#bank1<-exp(bank1)
#bank1<-exp(bank1)
...
for(ti in 2:5)
{
    ...
    for(si in st)
    {
        ...
        #if(ti==3)
        #{
            #tsam<-arima(ts,order=or,xreg=data.frame(xregfit),io=c(108))
        #}else
        if(ti==4)
        {
        ...
        #if(ti==6)
        #{
            #tsam<-arima(ts,order=or,xreg=xregfit,io=c(33))
        #}else if(ti==7)
        #{
            #tsam<-arima(ts,order=or,xreg=xregfit,io=c(137))
        #}else if(ti==8)
        #{
            #tsam<-arima(ts,order=or,xreg=data.frame(xregfit),io=c(86))
        #}else
        #{
            #tsam<-arima(ts,order=or,xreg=xregfit)
        #}
        ...
        while(needFix)
        {
            ...
            if(needFix)
            {
                ...
                #if(ti==3)
                #{
                    #tsam<-arima(ts,order=or,xreg=data.frame(xregfit),io=c(108),fixed=fixedv,transform.pars = FALSE)
                #}else
                if(ti==4)
                {
                    tsam<-arima(ts,order=or,xreg=xregfit,fixed=fixedv,transform.pars = FALSE,io=c(33))
                }else if(ti==5)
                {
        ...
        #if(ti==6)
        #{
            #tsam<-arima(ts,order=or,xreg=xregfit,io=c(33),fixed=fixedv,transform.pars = FALSE)
        #}else if(ti==7)
        #{
            #tsam<-arima(ts,order=or,xreg=xregfit,io=c(137),fixed=fixedv,transform.pars = FALSE)
        #}else if(ti==8)
        #{
            #tsam<-arima(ts,order=or,xreg=data.frame(xregfit),io=c(86),fixed=fixedv,transform.pars = FALSE)
        #}else
        #{
            #tsam<-arima(ts,order=or,xreg=xregfit,fixed=fixedv,transform.pars = FALSE)
        #}
    ...
    #if(ti==3)
    #{
        #tsam.bestfit<-arima(ts,order=or,xreg=data.frame(xregfit),io=c(108),fixed=fixb,transform.pars = FALSE);
    ...
    if(ti==4)
    {
        tsam.bestfit<-arima(ts,order=or,xreg=xregfit,fixed=fixb,transform.pars = FALSE,io=c(33));
        fittedValueTmp<-fitted(tsam.bestfit)
        print(tsam.bestfit);
        tsam.bestfit$coef<-tsam.bestfit$coef[1:(length(tsam.bestfit$coef)-1)]
    }else if(ti==5)
    ...
    }else
    {
        ...
        #if(ti==6)
        #{
            #tsam.bestfit<-arima(ts,order=or,xreg=xregfit,io=c(33),fixed=fixb,transform.pars = FALSE)
            #fittedValueTmp<-fitted(tsam.bestfit)
            #print(tsam.bestfit);
            #tsam.bestfit$coef<-tsam.bestfit$coef[1:(length(tsam.bestfit$coef)-1)]
        #}else if(ti==7)
        #{
            #tsam.bestfit<-arima(ts,order=or,xreg=xregfit,io=c(137),fixed=fixb,transform.pars = FALSE)
            #fittedValueTmp<-fitted(tsam.bestfit)
            #print(tsam.bestfit);
            #tsam.bestfit$coef<-tsam.bestfit$coef[1:(length(tsam.bestfit$coef)-1)]
        #}else if(ti==8)
        #{
            #tsam.bestfit<-arima(ts,order=or,xreg=data.frame(xregfit,AO=1*(seq(ts)==86)),io=c(85),fixed=fixb,transform.pars = FALSE)
            #tsam.bestfit<-arima(ts,order=or,xreg=data.frame(xregfit),io=c(86),fixed=fixb,transform.pars = FALSE)
            #fittedValueTmp<-fitted(tsam.bestfit)
            #print(tsam.bestfit);
            #tsam.bestfit$coef<-tsam.bestfit$coef[1:(length(tsam.bestfit$coef)-1)]
        #}else
        #{
            #tsam.bestfit<-arima(ts,order=or,xreg=xregfit,fixed=fixedv,transform.pars = FALSE)
            #fittedValueTmp<-fitted(tsam.bestfit)
            #print(tsam.bestfit);
        #}
    #tsam.bestfit<-arima(ts,order=or,xreg=xregfit,fixed=fixb,transform.pars = FALSE);
